...
function BudgetsPage() {
  ...
  const { data: cats = [] } = useQuery({
    ...
    enabled: !!user,
    queryFn: async () =>
      ((await listCategories()) as Category[]).filter((c) => c.kind === "expense"),
  });

  const { data: budgets = [] } = useQuery({
    queryKey: ["budgets", user?.id, month, year],
    enabled: !!user,
    queryFn: async () => (await listBudgets({ data: { month, year } })) as Budget[],
  });
  ...
  const { data: spent = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const rows = await listTransactions({
        data: { from: start, to: end, limit: 1000, offset: 0 },
      });
      return rows as { category_id: string | null; amount_cents: number; type: string }[];
    },
  });
  ...
  const buildBudgetRows = () =>
    [...budgets]
      .sort((a, b) => {
        const pa = (spentMap.get(a.category_id) ?? 0) / a.amount_cents;
        const pb = (spentMap.get(b.category_id) ?? 0) / b.amount_cents;
        return pb - pa;
      })
      .map((b) => {
        const c = catMap.get(b.category_id);
        const used = spentMap.get(b.category_id) ?? 0;
        const limit = b.amount_cents;
  ...
  const handleExportCSV = () => {
    ...
    const rows = buildBudgetRows().map((r) => [
      r.name,
      centsToInput(r.limit),
      centsToInput(r.used),
      centsToInput(r.remaining),
      `${r.pct.toFixed(1)}%`,
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[...budgets]
            ...
            .map((b) => {
              const c = catMap.get(b.category_id);
              const used = spentMap.get(b.category_id) ?? 0;
              const pct = Math.min(100, b.amount_cents > 0 ? (used / b.amount_cents) * 100 : 0);
              const over = used > b.amount_cents;
              const barColor =
                pct > 90 ? "var(--flare)" : pct > 70 ? "var(--info)" : "var(--primary)";
              return (
                <BrutalCard key={b.id} className="p-5">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button
                      onClick={() => confirm("Remover?") && del.mutate(b.id)}
                      className="text-muted-foreground hover:text-[color:var(--flare)]"
                    >
                  ...
                  <div className="flex items-end justify-between mb-2 font-mono text-sm">
                    ...
                    <span className="text-muted-foreground">/ {brl(b.amount_cents)}</span>
                  </div>
...
function BudgetModal({
  ...
  const save = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await upsertBudget({ data: { categoryId, month, year, amountCents } });
      ...
      >
        ...
        <div className="flex gap-2 pt-2">
          ...
          <button
            ...
            className="flex-1 bg-primary text-primary-foreground py-2.5 text-xs uppercase tracking-wider font-medium disabled:opacity-50"
          >
            {saving ? "..." : "Salvar"}
          </button>
        </div>
